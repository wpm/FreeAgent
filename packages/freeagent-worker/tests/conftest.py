"""Test configuration and fixtures for the Free Agent worker's tests.
...
puts *this* directory back on the path so a test's bare sibling-helper imports resolve.
...
SDK's own ``nats_server`` fixture (ADR-0008: integration tests run against a real server, not a
